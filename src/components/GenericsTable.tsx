...
const GenericTable = <T extends { id: string | number }>({ data, columns, itemsPerPage = 10, searchKey }: TableProps<T>) => {
                                            ...
                                            <td colSpan={columns.length} className="py-2 px-4 border-b">
                                                <div className="flex flex-col">
                                                    {columns.slice(1).map((col, colIndex) => (
                                                        <div key={colIndex}>
                                                            <strong>{col.header}:</strong> {col.render ? col.render(item) : String(item[col.key as keyof T])}
                                                        </div>
                                                    ))}
                                                </div>
                                            </td>
